...
use cursive::{
    ...
    theme::{BorderStyle, Color, ColorStyle, Effect, Palette, PaletteStyle, Theme},
    view::CannotFocus,
...
impl View for BoardView {
    ...
    fn on_event(&mut self, ev: Event) -> EventResult {
        match ev {
            ...
            Event::Key(key @ (Key::Up | Key::Down | Key::Left | Key::Right)) if !self.solved => {
                let delta = match key {
...
impl TheCoolerSliderView {
    pub fn new<S: Into<String>>(title: S, min: u32, max: u32, start: u32) -> Self {
        if !(max >= min) {
            panic!(
